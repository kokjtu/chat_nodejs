...
exports.create = function (name, email, username, password, callback) {
  exports.findByUsername(username, function (err, user) {
    if (user) callback('username already exists !')
    else {
...
exports.findById = function (id, callback) {
  User.findOne({ _id: id }, function (err, user) {
    if (!user) callback('User ' + id + ' does not exist')
...
exports.findByUsername = function (username, callback) {
  User.findOne({ username: username }, function (err, user) {
    if (!user) callback('User ' + username + ' does not exist')
    else callback(null, user)
  })
}
...
module.exports.delMessages = function (idTo, idFrom, callback) {
  User.findByIdAndUpdate(
    idFrom,
    { $pull: { messagesTo: { idTo: idTo }, messagesFrom: { idFrom: idTo } } },
